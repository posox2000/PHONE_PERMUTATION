...
export class PhoneService {
    ...
    getPage(phoneNum: string, pagesize: number, page: number) {
     ...
     private foundIndexes(number: string[], startElement: number, ind: number) {
        this.startIndexes[ind] = startElement % (this.letters[number[ind]].length);
        if (Math.floor(startElement / (this.letters[number[ind]].length)) === 0) {
          return;
        }
        this.foundIndexes(number, Math.floor(startElement / (this.letters[number[ind]].length)), ++ind);
     }
}
